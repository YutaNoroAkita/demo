...
初期データとしてタスクが3件入っているので、すぐにバグを体験できる：

1. タスクを追加 → **入力欄がクリアされない（Bug 1）**
2. タスクを完了にする → **Done カウンターが変わらない（Bug 2）**
3. タスクを削除する → **別のタスクが消える（Bug 3）**

> **Bug 3 の再現条件**: `deleteTask()` は「`id + 1`」を探すため、**末尾以外**のタスク（例: 1件目「APIの認証バグを修正」）を削除すると、**次の**タスクが消えます。逆に末尾のタスクを削除しても `id + 1` が存在せず何も起きないので、デモでは末尾以外を削除してください。
